...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="xl-data" as="node()*"></xsl:variable>
    ...
    <xsl:template name="start_excel">
        <xsl:apply-templates select="$xl-data"></xsl:apply-templates>
    </xsl:template>
    <xsl:variable name="colNames">
        ...
        <xsl:for-each select="$xl-data/worksheet/sheetData/row[1]/c">
            <xsl:message select="normalize-space(translate(@r,'1',''))"></xsl:message>
