...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="xl-root"
        select="'file:///C:/Users/hlapin/Box%20Sync/Research/Dicta%20Alignment%20Experiments/dicta_collatex/'"/>
    ...
    <xsl:variable name="target" select="'venice'"/>
    <xsl:variable name="compare1" select="'yAkad'"/>
    <xsl:variable name="compare2" select="'yVenSefaria'"/>
    <xsl:variable name="line_match_pattern" select="'-eSc_line_'"/>
    ...
    <xsl:variable name="files" select="file:list($xl-root, true(),'*.xlsx')"/>
    ...
    <xsl:template name="start">
        ...
        <out>
            <!-- get lines-to-process from excel -->
            <xsl:for-each select="$xml-data">
                ...
                <!--<!-\- for debugging -\->
                <xsl:copy-of select="key('line','465-eSc_line_448b9500',$lines-to-use)"></xsl:copy-of>
                <xsl:copy-of select=".//tei:l[matches(@corresp,'465-eSc_line_448b9500')]"></xsl:copy-of>-->
