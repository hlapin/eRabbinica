...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="morph-source"
        select="'zip:file:/C:/Users/hlapin/Documents/GitHub/mishnah/xsl-external/IntermediateMekMorph.xlsx!/xl/worksheets/sheet1.xml'"/>
    ...
    <xsl:template name="start">
        <TEI>
            ...
            <text>
                ...
                <body> 
                    <div> 
                        <xsl:for-each select="$morph-data//*:row">
                            <entryFree
                                xml:id="{local:ascii2}"
                                n="{local:ascii}">
                                <w lemma="{local:translit(local:lexeme)}">
                                    <seg type="uncorrected-surface-values">
                                            <span n="some_identifier">
                                                <xsl:value-of select="local:surface"/>
                                            </span>
                                    </seg>
                                    <span type="lemma">
                                        <xsl:value-of select="local:lexeme"/>
                                    </span>
                                    <span type="vocalized-surface">
                                        <xsl:value-of select="replace(local:tokenized, '\|', '')"/>
                                    </span>
                                    <span type="unvocalized-surface">
                                        <xsl:value-of select="replace(replace(local:tokenized, '\|', ''), $niqqud, '')"/>
                                    </span>
                                    <choice>
                                        <xsl:variable name="prefixes" select="
                                                (local:waw | local:article | local:interrogat |
                                                local:preposition | local:relative | local:temporal |
                                                local:adverbial)"/>
                                        <xsl:variable name="mains" select="
                                                (local:POS | local:genus | local:numerus | local:person | 
                                                local:status | local:tense | local:binyan)"/>
                                        <xsl:variable name="suffixes" select="
                                                (local:suffix_function | local:suffix_genus | 
                                                local:suffix_numerus | local:suffix_person)"/>
                                        <seg>
                                            <xsl:if test="$prefixes">
                                                <m function="pref" ana="{for $p in $prefixes 
                                                    return key('dict',$p/text(),$dict-data)/local:values}"
                                                    n="{$prefixes}">
                                                  <xsl:value-of
                                                  select="substring-before(local:tokenized, '|')"/>
                                                </m>
                                            </xsl:if>
                                            <m function="main"
                                                ana="{for $m in $mains 
                                                ...
                                                n="{$mains}">
                                                <xsl:value-of select="
                                                        if (not(contains(local:tokenized, '|'))) then
                                                            local:tokenized
                                                        else
                                                        substring-after(local:tokenized, '|')"/>
                                            </m>
                                            <xsl:if
                                                test="local:suffix_function/normalize-space()">
                                                <m function="suff"
                                                  ana="{for $s in $suffixes
                            ...
                            </entryFree></xsl:for-each>
                             <!--<!-\\-<raw><xsl:copy-of select="."></xsl:copy-of></raw>-\\->-->
