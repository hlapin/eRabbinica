...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="@* | element()" mode="production">
        <xsl:copy>
    ...
    <xsl:template match="element()[descendant::*[(self::pb | self::cb)]]" mode="production">
        <xsl:apply-templates mode="production"/>
    </xsl:template>
    <xsl:template match="element()[not(descendant-or-self::pb | descendant-or-self::cb)]"
        mode="production"> </xsl:template>
