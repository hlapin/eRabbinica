...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="src" select="'talmud'"><!-- bible or talmud --></xsl:param>
        ...
        <teiHeader>
            <fileDesc>
                ...
                <publicationStmt>
                    ...
                    <idno type="local">ref-bible</idno>
    ...
    <xsl:template match="local:bibleRabbRefValues">
        <TEI>
            <xsl:choose>
    ...
    <xsl:template match="tei:div[@subtype='mishnah']"></xsl:template>
    <xsl:template match="tei:div[@subtype = 'folio' or @subtype = 'chapter']">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$src='talmud'">
                <ab xml:id="{concat($prefix,'.',@n)}"><xsl:call-template name="doFolioWds">
                    <xsl:with-param name="prefix" select="concat($prefix,'.',@n)"/>
                </xsl:call-template></ab>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="doFolioWds">
        <xsl:param name="prefix"></xsl:param>
        <xsl:variable name="toTokenize">
            <xsl:variable name="str">
                <xsl:apply-templates select="*[not(@subtype='mishnah')]"/>
            </xsl:variable>
            ...
            <xsl:apply-templates select="$tokens">
                <xsl:with-param name="wPrefix" select="$prefix"/>
            </xsl:apply-templates>
    </xsl:template>
    <xsl:template match="tei:ab[@subtype = 'mishnah']"/>
    <xsl:template match="tei:ab[parent::*/@subtype='bavli']">
    ...
    <xsl:template match="tei:*[not(self::tei:w)and ancestor::tei:ab]">
        <xsl:if test="$src = 'bible'"><xsl:copy-of select="."></xsl:copy-of></xsl:if>
    </xsl:template>
    <xsl:template match="tei:add|tei:del">
        <xsl:value-of select="if (self::tei:add) then '[' else ']'"/>
        <xsl:value-of select="."/>
        <xsl:value-of select="if (self::tei:add) then ']' else '['"/>
    </xsl:template>
</xsl:stylesheet>
