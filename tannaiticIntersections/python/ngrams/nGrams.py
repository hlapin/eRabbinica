...
def sortByFreq(freqList, beg, end):
    sortedGrams = []
    sortedGrams = sorted(freqList.items(),\
...
def replaceNGramWToken(strFile,nGrFile):
    print(strFile,nGrFile)
    
    #empties the reduced text file to start over.
    open(strFile + "-reduced.txt","w", encoding="utf8").close()

    # f1 = open(strFile + ".txt", "r", encoding="utf8")
    # text = f1.read()
    # f1.close()

    with open(strFile + ".txt", "r", encoding="utf8") as f1:
        text = f1.read()
        text = ' '.join(text.split()) # deal with spaces?
    print(text[:100])
    
    with open(nGrFile + ".txt", "r", encoding="utf8") as f2:
    ...
    with open(strFile + "-reduced.txt","w", encoding="utf8") as f3:
        f3.write(text)
...
#idListF = file of original tokens with ID
def reducedTokenList(plainTxtList, idListF):
    print(idListF + ".txt")
    print(plainTxtList[:15])
    with open(idListF + ".txt", encoding="utf8") as idList:
