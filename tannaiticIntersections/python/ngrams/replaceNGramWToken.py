...
#'parameters'
plainTxtF = "ref-y-plain"
tokensListWIds = "ref-y"
nGramsList = ""
...
open(consolid + ".txt","w",encoding="utf8").close()
...
for i in range(nEnd, n - 1, -1):
	...
	with open(nGramsList + ".txt", "r", encoding="utf8") as fIn:
        # why was this inserting double line breaks?
		tempTest = fIn.read().replace("\n\n","\n")
		with open(consolid + ".txt","a", encoding="utf8") as f2:
			f2.write(tempTest)
...
with open(tokensListWIds + "-reduced.txt",'w+', encoding="UTF-8") as outFile :
