...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <div type="table">
            <table>
                <xsl:apply-templates select="TEI/text/body//list"/>
            </table>
            <div type="notes">
    ...
    <xsl:template match="/TEI/text/body/p"></xsl:template>
    <xsl:template match="/TEI/text/body/list|/TEI/text/body/div/list">
        <xsl:apply-templates/>
    </xsl:template>
    <xsl:template match="item">
        <row>
            <cell type="lemma" n="{count(preceding-sibling::item) + 1}">
                <xsl:value-of select="parent::list/preceding-sibling::p[1]/hi[last()]"/>
            </cell>
            <xsl:for-each-group select="node()" group-starting-with="hi[matches(normalize-space(.),'(O:|Ds:|F:|S:|E:|D:)')]">
                <cell type = "{normalize-space(substring-before(current-group()[1], ':'))}">
                    <xsl:apply-templates select="current-group()[not( . is current-group()[1])]"></xsl:apply-templates>
                </cell>
            </xsl:for-each-group>
        </row>
    </xsl:template>
