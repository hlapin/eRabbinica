...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
            ...
            <!--<xsl:result-document encoding="UTF-8" href="{concat($path,$parsedTitle[1],'.xml')}">-->
                <TEI>
                    <teiHeader>
                        <fileDesc>
                            <titleStmt>
            ...
            <div type="name-list">
                <xsl:for-each select=".//table">
                    <xsl:variable name="nameTranslit"
                        select="
                        normalize-space(if (matches(preceding-sibling::*[1]/(text() | hi)[last()], '[&#x2212;&#x2013;] ')) then
                        tokenize(preceding-sibling::*[1]/(text() | hi)[last()], '[&#x2212;&#x2013;]')[2]
                        else
                        preceding-sibling::*[1]/(text() | hi)[last()])"/>
                    <xsl:variable name="nameOrig"
                        select="normalize-space(replace(string-join(preceding-sibling::*[1]/(text() | hi)[1]), '[&#x2212;&#x2013;]', ''))"/>
    ...
    <xsl:template match="list[not(ancestor::cell)]">
       <xsl:param  name="parsed-URI" tunnel="yes"></xsl:param>
        ...
        <ab type="name" n="{$nameTranslit}-{$nameOrig}">
            ...
            <listPerson xml:id="{$nameTranslit}-{$parsed-URI[self::prefix]}" n="{$nameOrig}">
    ...
    <xsl:template match="item">
        ...
        <!-- commenting out instructions to create new file for each person. Do this after second pass. -->
        <xsl:variable name="xid" select="concat($parsed-URI[self::prefix], '-', generate-id())"/>
        <!--<xsl:element name="xi:include">
            <xsl:attribute name="href" select="concat($to-file-out, $path-suffix)"/>
            <xsl:attribute name="xpointer" select="concat($xid, '-person')"/>
        </xsl:element>-->
    ...
    <xsl:template match="table">
        ...
        <listPerson xml:id="{translate($nameTranslit,' ','_')}-{$parsedfileName}" n="{$nameOrig}">
